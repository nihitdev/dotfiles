...
def validate_vendored_files() -> None:
    licenses = list((ROOT / "yazi/plugins").glob("*.yazi/LICENSE"))
    for path in licenses:
        text = path.read_text(encoding="utf-8")
        if "MIT License" not in text or len(text.splitlines()) < 20:
            error(path, "missing complete vendored license")

    duplicate_a = ROOT / "bat/themes/Catppuccin Mocha.tmTheme"
    duplicate_b = ROOT / "yazi/flavors/catppuccin-mocha.yazi/tmtheme.xml"
