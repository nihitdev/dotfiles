...
run_static_security_defaults_test() {
    ...
    grep -Fqx -- '--color=auto' "$repo_root/bat/config" || fail 'Bat does not use automatic color mode'
    if grep -Fq '~/.config/fastfetch/ascii.txt' "$repo_root/fastfetch/config.jsonc"; then
        fail 'Fastfetch config still hard-codes ~/.config'
    fi
    local license
    for license in "$repo_root"/yazi/plugins/*.yazi/LICENSE; do
